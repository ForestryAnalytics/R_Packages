...
---
title: "``forestmangr::plot_summarise``:  Summarize forest inventory data"
...
<pre><code>
plot_summarise(df, plot, plot_area, dbh, th, .groups, total_area, vwb, vwob, dh, age, dec_places = 4)
</code></pre>
...
dbh Optional parameter. Quoted name of the diameter at breast height variable. If supplied, will be used to calculate the mean diameter per plot, quadratic diameter
...
plot. Default NA.
dh Optional parameter. Quoted name of the dominant height variable. If supplied, will be used to calculate the mean dominant height per plot. If not, the ht
variable supplied will be used to calculate the average of the top two trees of each plot, and use that as dh. Default: NA.
