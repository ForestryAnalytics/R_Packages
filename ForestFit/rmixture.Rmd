...
where K is the number of components, $\theta_j$ for $ j = 1, \ldots,K$ is the parameter
space of the j-th component,
i.e. j = (j ; j)T , and $\Theta  = (\theta 1, \ldots, \theta_K)^T$ , is the whole parameter vector. 

Parameters $\alpha$ and $\beta$ are the shape and scale parameters or both are the shape parameters. 
In the latter case, the parameters $\alpha$ and $\beta$ are called the first and second shape parameters, respectively.

We note that the constants $\omega_{j}$s sum to one, i.e.
